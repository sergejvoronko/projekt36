...
- Leaves, grit, and pine needles collect in the sunroof tray
- Organic debris decomposes into a paste that plugs the drain opening
- The rubber hoses crack, collapse, or separate from their fittings with age
- The lower exit points can be blocked by mud or accumulated road debris

The front drains are the most commonly blocked because the front tray collects more debris. The rear drains tend to stay cleaner but are more prone to hose deterioration.
...
### Wet rear parcel shelf / boot

Blocked rear drains send water down into the boot area or through the C-pillar:
- Wet C-pillar trim (often mouldy-smelling without obvious wet carpet)
- Damp boot carpet or liner
- Water pooling under the boot floor panel

### Electrical damage

This is where blocked sunroof drains become expensive:

**ZKE module (in the boot, right side):** The module sits in an area where rear drain overflow collects. Water corrodes the PCB and connector pins. Symptoms: random central locking, remote stops working, one-touch windows fail. The module appears to work until corrosion causes an intermittent fault.

**Instrument cluster:** Front drain overflow runs toward the A-pillar and can reach the instrument cluster wiring behind the dash. Corroded instrument cluster connectors cause erratic gauge behaviour, missing segments, and communication faults on the K-Bus.
...
With the sunroof open (or tilted), look into the drain channels at the four corners of the tray. They're small rectangular slots or openings with the drain hose fitted over the spigot below. Accumulated debris should be visible if the drain is blocked. Clean out with a soft brush or compressed air.
...
A flexible drain clearing wire (as used for washing machine drain hoses) or a piece of trimmer line can be carefully pushed down the drain hole to break up blockages. Don't use anything rigid that could pierce the hose.
...
Replacement drain hose is standard 8 mm ID rubber hose (or slightly larger depending on year). Buy 2 metres, cut to length. The hose connects over the tray drain spigot at the top and exits at the lower grommet point.
...
| Entry point | Wet area | Distinguishing sign |
|-------------|----------|---------------------|
| Sunroof drain (front) | A-pillar, front footwell | Only leaks during/after rain; dry in summer |
| Sunroof drain (rear) | C-pillar, boot | Same pattern, rear end |
| Windscreen seal | Centre dash, upper footwell | Water tracks down from screen surround |
| Door seal | Door sill, lower door trim | Water entry around door glass |
| Pollen filter housing | Passenger footwell | Specific to heavy rain with blocked pollen tray |
| Boot seal (saloon) | Boot floor | Water pools in boot without C-pillar wetness |

The pollen filter / cabin air intake is a common secondary source on E36s: the tray under the pollen filter housing (at the base of the windscreen in the engine bay) can fill up if its own drain is blocked, and overflow into the passenger footwell through the firewall grommet. This presents identically to a front sunroof drain leak and is worth checking at the same time.
